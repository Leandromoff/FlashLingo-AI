...
const rawText = fs.readFileSync('data_en_voc_l1.txt', 'utf8');
...
for (let line of lines) {
    ...
    if (parts.length >= 3) {
         ...
         if (wtParts.length >= 2) {
              ...
              items.push({
                  id: 'lesson_1_vocabulary_a2_card_' + (cardCounter++),
                  word: word,
                  translation: trans,
                  pronunciation: ipa,
...
fs.writeFileSync('public/decks/lesson_1_vocabulary_a2.json', JSON.stringify(items, null, 2));
console.log('Created lesson_1_vocabulary_a2.json with ' + items.length + ' cards');

let meta = JSON.parse(fs.readFileSync('data/decksMetadata.json', 'utf8'));

if (!meta['lesson_1_vocabulary_a2']) {
  meta['lesson_1_vocabulary_a2'] = {};
}
meta['lesson_1_vocabulary_a2'].en = items.length;
