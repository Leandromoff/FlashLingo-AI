...
const inputDecks: Record<string, SentenceInput[]> = {
  ...
  homework_29: [
    ...
    { word: "Don’t drink coffee at night.", translation: "Don’t drink coffee at night." },
  ...
  homework_41: [
    ...
    { word: "She is my mother. Her name is Kim.", translation: "Ela é minha mãe. O nome dela é Kim." },
...
async function generateChunkData(deckId: string, cards: SentenceInput[], chunkIndex: number, totalChunks: number, startIndex: number): Promise<any[]> {
...
8. exampleSentence: Equal to "word"
9. exampleTranslation: Equal to "translation"
...
This is batch ${chunkIndex + 1} of ${totalChunks} (items starting from sequential index ${startIndex + 1}).
Input chunk:
${JSON.stringify(cards, null, 2)}
`;

  console.log(`[${deckId}] Requesting chunk ${chunkIndex + 1}/${totalChunks}...`);
  ...
  if (!text) {
    throw new Error(`Empty response from Gemini for ${deckId} chunk ${chunkIndex}`);
  }
...
async function run() {
  ...
  if (!deckId || !inputDecks[deckId]) {
    console.error(`Invalid deckId. Please specify one of: ${Object.keys(inputDecks).join(", ")}`);
    process.exit(1);
  }

  const cards = inputDecks[deckId];
  const dataDir = path.join(process.cwd(), "data");
  const targetFile = path.join(dataDir, `${deckId}.json`);

  // Chunk cards into batches of max 9 cards
  const CHUNK_SIZE = 9;
  ...
  console.log(`[${deckId}] Splitting ${cards.length} cards into ${chunks.length} chunks of size <= ${CHUNK_SIZE}...`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    while (!success && attempts < 5) {
      try {
        attempts++;
        const generatedChunk = await generateChunkData(deckId, chunk, i, chunks.length, startIndex);
        
        // Assert correct format of ID
        for (let j = 0; j < generatedChunk.length; j++) {
          generatedChunk[j].id = `${deckId}_${startIndex + j + 1}`;
        }

        allGenerated.push(...generatedChunk);
        console.log(`SUCCESS: Chunk ${i + 1}/${chunks.length} generated.`);
        success = true;
      } catch (err: any) {
        console.error(`Attempt ${attempts} failed for chunk ${i + 1}:`, err?.message || err);
        if (attempts < 5) {
          const timeoutDur = err?.status === 429 ? 35000 : 8000;
          console.log(`Waiting ${timeoutDur / 1000} seconds before retrying chunk...`);
          await wait(timeoutDur);
        } else {
          console.error(`FATAL: Failed to generate chunk ${i + 1} after 5 attempts.`);
          process.exit(1);
        }
      }
    }

    // Space-limiting between chunks
    if (i < chunks.length - 1) {
      console.log("Spacing delay of 3.5 seconds to protect rate limits...");
      await wait(3500);
    }
  }

  // Double check list size matches
  if (allGenerated.length !== cards.length) {
    console.error(`FATAL: Mismatched list size. Expected ${cards.length} cards, got ${allGenerated.length}`);
    process.exit(1);
  }

  fs.writeFileSync(targetFile, JSON.stringify(allGenerated, null, 2));
  console.log(`SUCCESS: Successfully wrote all ${allGenerated.length} cards to /data/${deckId}.json`);
  process.exit(0);
}
