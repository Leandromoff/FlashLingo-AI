...
const rawText = fs.readFileSync('data_es_l3.txt', 'utf8');
...
for (let line of lines) {
    ...
    if (parts.length >= 3) {
         ...
         if (wtParts.length >= 2) {
              ...
              items.push({
                  id: 'leccion_3_gramatica_a2_s_card_' + (cardCounter++),
                  word: word,
                  translation: trans,
                  pronunciation: ipa,
...
fs.writeFileSync('public/decks/leccion_3_gramatica_a2_s.json', JSON.stringify(items, null, 2));
console.log('Created leccion_3_gramatica_a2_s.json with ' + items.length + ' cards');

let meta = JSON.parse(fs.readFileSync('data/decksMetadata.json', 'utf8'));

if (!meta['leccion_3_gramatica_a2_s']) {
  meta['leccion_3_gramatica_a2_s'] = {};
}
meta['leccion_3_gramatica_a2_s'].es = items.length;
