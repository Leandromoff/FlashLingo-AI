...
if (!typesData.includes("'to_be_intro'")) {
    typesData = typesData.replace(
        /label:\s*'35 Escrever, Pensar & Achar',\n\s*isStatic:\s*true\n\s*\}/, 
...
if (!staticDecksData.includes('export const TO_BE_INTRO_DECK')) {
    ...
    // In moveDeck we made STATIC_DECKS like this:
    // export const STATIC_DECKS: Record<string, Record<string, FlashcardData[]>> = {
    //   write_think: {
    //     en: WRITE_THINK_DECK
    //   },
    // So we can replace write_think mapping with write_think and to_be_intro.
    staticDecksData = staticDecksData.replace(/write_think: \{\n\s*en: WRITE_THINK_DECK\n\s*\}/, 
    "write_think: {\n    en: WRITE_THINK_DECK\n  },\n  to_be_intro: {\n    en: TO_BE_INTRO_DECK\n  }");
    
    // To ensure the array variable is defined before we use it, let's inject it right before STATIC_DECKS map.
    ...
    console.log('Deck TO_BE_INTRO rebuilt successfully!');
} else {
